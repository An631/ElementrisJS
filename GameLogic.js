document.addEventListener("DOMContentLoaded",function(){
    ...
    var brick_border = canvas.width*.05;
    var brick_size = canvas.width/max_cols - brick_border;
    Log(brick_border);
    Log(brick_size);
    ...
    function drawBricks() {
        for(var row = 0; row < max_rows; row++){
            for(var col = 0; col < max_cols; col++){
                var brickX = col*brick_size + brick_border/2;
                var brickY = row*brick_size + brick_border/2;
                context.beginPath();
                context.rect(brickX, brickY, brick_size, brick_size);
                var clr = Math.floor(Math.random()*6+1);
